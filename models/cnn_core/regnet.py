...
class ResBottleneckBlock(nn.Module):
    ...
    def forward_calc_flops(self, x: Tensor) -> Tensor:
        x, flops = x
        if self.proj is not None:
            identity = self.proj(x)
            flops += self.downsample_flops * identity.shape[2] * identity.shape[3]
        else:
            identity = x
        ...
        flops += (self.conv1_flops_per_pixel * h_conv1 * h_conv1 + self.conv2_flops_per_pixel * h * w + self.conv3_flops_per_pixel * h * w)
...
class AnyStage(nn.ModuleList):
    ...
    def forward_calc_flops(self, x):
        ...
        x = (x, flops)
        for block in self:
            x = block.forward_calc_flops(x)
        
        # x, flops = x
        return x    # should be a tuple of (x, flops)
...
class RegNet(nn.Module):
    def __init__(
        ...
        # print(block_params._get_expanded_params())
...
if __name__ == '__main__':
    net = regnet_y_400mf(pretrained=False)
